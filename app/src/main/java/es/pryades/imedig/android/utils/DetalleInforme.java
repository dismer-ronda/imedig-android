...
public class DetalleInforme extends Informe
{
	...
	public String getCentroImagenUrl()
	{
		...
		String token = "token=" + Utils.getTokenString( centro_imagen + "" + ts, Constants.TrustKey );
		String code = "code=" + Utils.encrypt( extra, Constants.TrustKey ) ;
		
		return StringEscapeUtils.escapeXml11( "imedig-services/image/" + centro_imagen + "?" + token + "&" + code );
	}

	public String toString()
	{
		return Utils.getDateFromLongAsString( getFecha(), "yyyy-MM-dd HH:mm" ) + " - " + getPaciente_nombre();
	}
}
